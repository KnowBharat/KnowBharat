...
export default function MapIndex() {
  ...
  flatExploredNodes.forEach(node => {
    ...
    if (parts.length === 2) {
      const stateKey = parts[0]; // This is a STRING ("14")
      const lvl = parseInt(parts[1], 10);
      if (!exploredStates[lvl]) exploredStates[lvl] = [];
      exploredStates[lvl].push(parseInt(stateKey, 10)); // Pushing the STRING "14"
    }
  });
  ...
  const selectState = useCallback((key) => {
    ...
    if (stateData[key]?.name) {
      apiFetch('/dashboard/activity', {
        method: 'POST',
        body: JSON.stringify({
          game: 'map',
          score: null,
          stateName: `Clicked ${stateData[key].name} (Lvl ${activeLevel || 1})`
        })
